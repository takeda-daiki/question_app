...
import { cardImageMarkdown, deleteCardImages, uploadCardImage } from "../images/storage";
    ...
    if (
      ...
      if (!cardCreated && uploadedImages.length) {
        try {
          ...
          // 保存前に閉じる際の掃除失敗は、入力を閉じられなくするほどではない。
        }
      }
      close();
    }
    ...
    >
      <form
        onSubmit={async (e) => {
          ...
          else attempt.current.title = clean;
          try {
            ...
            <div className="form-grid">
              <label>
                ...
                <select
                  ...
                  disabled={busy || creating || imageBusy || cardCreated}
                  onChange={(e) =>
                    setStatus(e.target.value as "unresolved" | "resolved")
                  }
                >
